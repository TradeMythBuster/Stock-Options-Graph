...
class remove:
	def __init__(self, master, value, first, second, third, fourth, fifth, database):
		...
		self.myframe.configure(background='black')
		...
		Label(self.myframe,text=first +'\t'+ second +'\t'+ third +'\tPremium : '+ fourth + '\tQuantity : '+ fifth,
			font=("Calibri", 13),bg='grey',fg='black').grid(row=self.number+2,column=0,columnspan = 4,padx=5,pady = 5,stick = W)
...
def add_strat(database):
	...
	Label(screen1,text = "Choose action (BUY/SELL) : ",padx = 5,pady = 5,font=("Calibri", 13),bg='black',fg='white').grid(row=0,column = 0)
	buy_sell = ttk.Combobox(screen1, value=["BUY", "SELL"],width=6)
	buy_sell.current(0)
	buy_sell.grid(row=0,column = 1,padx = 5,pady = 5)

	Label(screen1,text = " "*15+"Choose type (CALL/PUT) : ",padx = 5,pady = 5,font=("Calibri", 13),bg='black',fg='white').grid(row=0,column = 2)
	ce_pe = ttk.Combobox(screen1, value=["CALL", "PUT"],width=6)
	ce_pe.current(0)
	ce_pe.grid(row=0,column = 3,padx = 5,pady = 5)

	Label(screen1,text = " "*15+"Strike Price : ",padx = 5,pady = 5,font=("Calibri", 13),bg='black',fg='white').grid(row=0,column = 4,padx = 5,pady = 5)
	strike = DoubleVar()
	strike_entry = Entry(screen1, textvariable = strike,bg='grey',fg='black')
	strike_entry.grid(row=0,column = 5,padx = 5,pady = 5)

	Label(screen1,text = " "*15+"Premium : ",padx = 5,pady = 5,font=("Calibri", 13),bg='black',fg='white').grid(row=0,column = 6,padx = 5,pady = 5)
	premium = DoubleVar()
	premium_entry = Entry(screen1, textvariable = premium,bg='grey',fg='black')
	premium_entry.grid(row=0,column = 7,padx = 5,pady = 5)

	Label(screen1,text = " "*15+"Quantity : ",padx = 5,pady = 5,font=("Calibri", 13),bg='black',fg='white').grid(row=0,column = 8,padx = 5,pady = 5)
	quantity = IntVar()
	quantity_entry = Entry(screen1, textvariable = quantity,bg='grey',fg='black')
	quantity_entry.grid(row=0,column = 9,padx = 5,pady = 5)

	add_leg_button = Button(screen1,text='Add Option Leg',padx = 5,pady = 5,font=("Calibri", 13),bg='grey',command=lambda:add_leg(database))
	add_leg_button.grid(row=1,column=0)

	create_graph_button = Button(screen1,text='Create Graph',padx = 5,pady = 5,font=("Calibri", 13),bg='grey',command=lambda:create_graph(database))
	create_graph_button.grid(row=1,column=2)

	save_button = Button(screen1,text=' Save Strategy ',padx = 5,pady = 5,font=("Calibri", 13),bg='grey',command=lambda:save_db(database))
...
heading = Label(root, text="Options Pay-Off Graph Generator",bg = 'black',fg='grey', height = 2, font=("Calibri", 18))#bg="grey"
heading.grid(row=0, column=0, columnspan=2, stick=W+E)

create = Button(root, text=" Create New Strategy ",bg='grey', padx = 5, pady = 5, font=("Calibri", 13), command = lambda:add_strat('trial.db'))
create.grid(row=1, column=0, pady=15, padx = 6.25)

open_saved = Button(root, text=" Open Saved Strategy ",bg='grey', padx = 5, pady = 5, font=("Calibri", 13), command = open_file)
